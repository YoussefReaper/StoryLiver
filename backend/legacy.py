...
from . import (authority, awareness, db, memory, narrgraph, relationships,
               worldstate)
...
def drift(pt_id, world, player=memory.SOLO, *, only_turned=False) -> list:
    ...
    for npc in world.npcs:
        ...
        row = db.row("SELECT cause_node, cause_turn FROM relationships"
                     " WHERE playthrough_id=? AND src=? AND dst=?", (pt_id, player, nid))
        cause = None
        if row and row["cause_node"]:
            n = narrgraph.node(pt_id, row["cause_node"])
            if n:
                cause = {"node_id": n["id"], "turn": n["turn"], "kind": n["kind"],
...
def found_org(pt_id, world, *, player, name, kind="cell", charter="", turn=0,
    ...
    db.run("INSERT INTO orgs (id,playthrough_id,founder,name,kind,charter,seat,founded_turn,"
           "created_at) VALUES (?,?,?,?,?,?,?,?,?)",
           (org_id, pt_id, player, name, kind, charter[:400], seat, turn, db.now()))
